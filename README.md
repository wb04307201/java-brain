...
> 整体有三部分组成 灵梭AI Agent + SQL工坊MCP + SQL工坊 组成，快速让你的系统具备智能能力
...
2. sql-forge-mcp通过stdio提供给spring-ai-chat-demo,目前spring-ai-chat-demo内已配置好stdio  
   但运行环境需要jbang(类似于python的uv，或者node的npx),安装命令可参考  
   > Windows (PowerShell)
   > ```shell
...
### 自然语言调用数据库
```text
请帮我查询订单系统数据库里有哪些商品？哪个商品在订单表里订购数量最多？哪个商品在订单表里订购价格合计最多？
```

也可以配合其他mcp工具例如继续对话：
```text
请更详细的分析订单系统里各种商品订购数量与销售额，并生成分析报告
```
