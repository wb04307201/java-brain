...
| 类型 | 数量 | 说明 |
|---|---:|---|
| fade_in 入场(一次性) | 65 | 1-2s 内完成,之后静止 |
| pulse_loop indefinite(含 5 chase) | 14 | 占满每页剩余时长,转视频核心;其中 5 个(P4b 2 + P5b 3)是 chase_pulse,各错开 2s |
| **总账** | **79** | 65 fade_in + 14 pulse_loop(chase 是 pulse_loop 的子集,不重复计数) |
| **indefinite 总数** | **14** | 所有 pulse_loop / chase_pulse 都带 `repeatCount="indefinite"` |
...
5. 每页 5±2 元素,留白≥20%
6. 79 个动画节点(65 fade_in + 14 pulse_loop,含 5 chase)
7. 14 个 pulse_loop(含 5 个 chase)用 `repeatCount="indefinite"`
