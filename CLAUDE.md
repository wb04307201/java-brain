...
JavaBrain 是这 3 个组件的组合演示，**上游开源仓库只有 2 个**（GitHub / Gitee 均已开源，各自独立维护、按需组合）：

| 仓库 | 包含组件 | 说明 |
|---|---|---|
| `spring-ai-loom-agent` | 灵梭 AI Agent | 独立可用，只做 AI 编排（RAG / MCP / Skill / 文件 / 对话 UI） |
| `sql-forge` | SQL 工坊 + SQL 工坊 MCP | **同一仓库不同模块**；SQL 工坊做数据管理（CRUD / Calcite 联邦 / Amis 低代码），SQL 工坊 MCP 做 AI ↔ DB 桥 |

> 组件 = 3 个（灵梭 / SQL 工坊 / SQL 工坊 MCP），仓库 = 2 个。
> 本仓库 `java-brain` 是组合演示载体（`oms` 业务底座 + `loom-agent` AI 前端），不是上述任意上游仓库的镜像。
